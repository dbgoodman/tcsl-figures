...
parent.dir <- '/Users/dbg/Library/CloudStorage/Box-Box/tcsl/incucyte/TCSL248/'
setwd(parent.dir)
```
...
filepaths.dt <- data.table(
  data.path=list.files(path=parent.dir, pattern='.+TCSL248.+txt', recursive=TRUE)
)
...
# Load plate maps
platemap.dt <- rbind(
  load_plate_metadata("plate_map_wk1.csv")[, week := 0],
  load_plate_metadata("plate_map_wk2.csv")[, week := 1])
...
wk0_data <- combined.data.dt[measurement == 'incucyte_total_int_intensity' & value > 0 & Elapsed < 80 & week == 0]
