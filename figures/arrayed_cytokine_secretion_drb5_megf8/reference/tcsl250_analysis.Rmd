...
data_dir <- '/Users/dbg/Library/CloudStorage/Box-Box/tcsl/flow/20241213.tcsl250 cytokines/'
```
...
```{r}
df_raw <- fread(file=paste0(data_dir, 'populations.csv'))
...
# Function to load and process a plate mapx
...
# Load plate maps
plate_map <- load_plate_metadata(paste0(data_dir, "plate_map.csv"))
...
```{r}
# Combine with TCSL248 data

df_tcsl248_melt <- fread( 
  "/Users/dbg/Library/CloudStorage/Box-Box/tcsl/flow/20241112_tcsl248_cytokine/processed_cytokine_data.csv")

df_tcsl248_melt
df_melt_combined <- rbind(
  df_melt[, expt := 'tcsl250'],
  df_tcsl248_melt[, expt := 'tcsl248'])

# cast 250 vs 248:
dcast(df_melt_combined, car_id + CAR + car_name + gate_clean + subset + parent_subset + parent_CAR + car_id ~ expt, value.var = "freq")
```
